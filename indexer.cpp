...
void word_tokenize(const string& s, vector<string>& tokens) {
    std::regex symbols("[^1-9A-Za-zÀ-ÿ']+");
    std::regex apostrophe_before_word("\\s'");
    std::regex apostrophe_after_word("'\\s");

    // Remove standalone apostrophes
    string s_without_apostrophes = regex_replace(s, apostrophe_before_word, " ");
    s_without_apostrophes = regex_replace(s_without_apostrophes, apostrophe_after_word, " ");

    stringstream ss(s_without_apostrophes);
    string token;
    while (getline(ss, token, ' ')) {
        if (token.empty()) continue;
        if (regex_search(token, symbols)) {
